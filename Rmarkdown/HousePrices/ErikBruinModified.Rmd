...
I started this competition by just focusing on getting a good understanding of the dataset. The EDA is detailed and many visualizations are included. This version also includes modeling. From the glmnet family of models, lasso performs best with regards to both the cross validation scores and public scores. The second model that I composed is an XGBoost model. Taking the average of those 2 models gives me the best public score at the moment.

I will continue to work on this analysis, and eventually also want to include stacking of models. If you like what I have done so far, upvotes are always appreciated :).
...
As you can see in Figure \@ref(fig:SP), the sale prices are right skewed. This was expected as few people can afford very expensive houses. I will keep this in mind, and take measures before modeling.

```{r, label = "SP", message = FALSE, fig.cap = "Sale Price in US dollars"}
...
It also becomes clear the multicollinearity is an issue. For example: the correlation between GarageCars and GarageArea is very high (`r round(cor_numVar[5, 4],2)`), and both have similar (high) correlations with `SalePrice`. The other 6 six variables with a correlation higher than 0.5 with `SalePrice` are:
...
Overall Quality has the highest correlation with SalePrice among the numeric variables (`r round(cor_numVar[2, 1],2)`). It rates the overall material and finish of the house on a scale from 1 (very poor) to 10 (very excellent).
...
In this section, I am going to fix the 34 predictors that contain missing values. I will go through them working my way down from most NAs until I have fixed them all. If I stumble upon a variable that actually forms a group with other variables, I will also deal with them as a group. For instance, there are multiple variables that relate to Pool, Garage, and Basement.

As I want to keep the document as readable as possible, I decided to use the "Tabs" option that `knitr` provides. You can find a short analysis for each (group of) variables under each Tab. You don't have to go through all sections, you can look at a subset of the tabs. If you do so, I think the Garage and Basement sections are worthwhile, as I have been careful in determining which houses really do not have a basement or garage.
...
Now, I can use the function `plyr::revalue()` to do the work for me.
...
Within Miscellaneous Feature (`MiscFeature`), there are 2814 NAs. As the values are not ordinal, I will convert `MiscFeature` into a factor. Values:	
...
A variable that can be made ordinal.
...
A variable that can be made ordinal with the FinType vector.
...
At this point, I have made sure that all variables with NAs are taken care of. However, I still need to also take care of the remaining character variables that are without missing values. Similar to the previous section, I have created Tabs for groups of variables.
...
2 variables that specify the flatness and slope of the property.
...
Although possibly a bit less steep than expected, the effects of the Banking crises that took place at the end of 2007 can be seen indeed. After the highest median prices in 2007, the prices gradually decreased.
...
Only 3 of those most important variables are categorical according to RF; `Neighborhood`, `MSSubClass`, and `GarageType`. Since I have already visualized `GrLivArea` and `OverallQuality` in my initial explorations, I will not visualize those predictors again in this section.
...
The size of the Basement is also ranked as an important variable. Similar to 2nd floor surface, I suspect that basement square feet might also be worth less (no windows, also needing stairs climbing/descending).

### The most important categorical variable; `Neighborhood`

Th first graph shows the median `SalePrice` by `Neighborhood`. The frequency (number of houses) of each Neighborhood in the train set is shown in the labels.

The second graph below shows the frequencies across all data.

```{r, warning = FALSE, out.width = "100%", fig.height = 6}
...
I have already visualized the relation between Overall Quality and `SalePrice` in my initial explorations, I want to visualize the frequency distribution as well. As there are more quality measurements, I am taking the opportunity to bundle them in this section.
...
As already mentioned in section 4.2, `GarageCars` and `GarageArea` are highly correlated. Here, `GarageQual` and `GarageCond` also seem highly correlated. However, both are dominated by `level = 3`. Therefore, I expect both of them to be dropped by algorithms that use regularization.
...
As expected, the graph shows a negative correlation with `Age` (old houses are worth less).
...
As far as I know, porches are sheltered areas outside of the house, and a wooden deck is unsheltered. Therefore, I am leaving `WoodDeckSF` alone, and will only consolidate the 4 porch variables.
...
For the time being, I am keeping it simple and just remove the two really big houses with low SalePrice manually. However, I intend to investigate this more in a later stage (possibly using the 'outliers' package).
...
## Composing `train1` and `test1` sets
...
Since the lasso and XGBoost algorithms are very different, averaging predictions likely improves the scores. As the lasso model does better regarding the cross validated RMSE score (`r min(lasso_mod$results$RMSE)` versus 0.1235), I am weighting the lasso model double.

Alan is doing a 30/30/40 split with RF, XGB, and lasso. -> Public score 0.12267 worse.  Next, trying a 40/60 split with XGB and lasso.

```{r}
sub_avg <- data.frame(Id = test_labels, SalePrice = (0.4*predictions_XGB + 0.60*predictions_lasso))
head(sub_avg)
write.csv(sub_avg, file = '3average.csv', row.names = F)
```

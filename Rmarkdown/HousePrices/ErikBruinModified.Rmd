...
ggplot(all[!is.na(all$LotFrontage),], aes(x = as.factor(Neighborhood), y = LotFrontage)) +
        ...
        labs(x = "", y = "Lot Frontage (linear feet of street connected to property")
```
...
Although the correlations are giving a good overview of the most important numeric variables and multicollinearity among those variables, I wanted to get an overview of the most important variables including the categorical variables before move on to visualization.
...
First of all, I am dropping a variable if 2 variables are highly correlated (such as GrLivArea and TotalRmsAbvGrd). 
...
In previous versions, I worked with Caret's `Near Zero Variance' function. Although this works, it also is a quick fix and too much information got lost. For instance, by using the defaults, all Neighborhoods with less than 146 houses are omitted as (one-hot encoded) variables (frequency ratio higher than 95/5). Therefore, I have taken a more carefull manual approach in this version.
...
Since the lasso and XGBoost algorithms are very different, averaging predictions likely improves the scores.
